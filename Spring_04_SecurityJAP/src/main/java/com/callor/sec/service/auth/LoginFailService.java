...
//        exception 의 정체를 파악하기 , type 오류는 발생하지 않는다.
         ...
         * POST /login URL 로 요청이 전달된다 , /login URL: security 에서 제공하는 login
         *
         * 로그인이 실패했을 때
         * request.getRequestDispatcher("/member/login")
         *                 .forward(request,response); 수행할때
         ...
         *
         * 만약 redirect를 했다면 GET 으로 호출이 된다.
         *
         */
        request.setAttribute("ERROR_MSG", error_msg);
//        request.setAttribute("AUTHOR", "callor@callor.com");

        // /member/login URL 로 forwarding 을 수행하라
        // redirect 가 아니고 forwarding
        request.getRequestDispatcher("/member/login?error") // login form이 있는 곳
                .forward(request,response); // 그 위에 있는 에러메세지를 같이 담고 가기 위함
